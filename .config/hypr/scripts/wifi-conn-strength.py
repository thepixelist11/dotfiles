...
if __name__ == "__main__":
    ...
    nm_conn_type = nm.Get(nm_iface, "PrimaryConnectionType", dbus_interface=props_iface)
    if nm_conn_type != "802-11-wireless":
        print("Primary connection is not Wi-Fi")
        exit(1)

    nm_conn_obj = nm.Get(nm_iface, "PrimaryConnection", dbus_interface=props_iface)
    nm_conn = bus.get_object(nm_bus, nm_conn_obj)
    nm_conn_iface = "org.freedesktop.NetworkManager.Connection.Active"

    nm_ap_obj = nm_conn.Get(nm_conn_iface, "SpecificObject", dbus_interface=props_iface)
